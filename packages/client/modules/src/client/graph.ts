/**
 * Browser-safe boot-graph composition: the bootstrap protocol ids and the
 * module-graph ordering rule. Zero node imports and zero dependencies, so a
 * host composer and an in-page composer derive identical row order from one
 * implementation instead of agreeing by convention.
 *
 * Row revisions are deliberately absent. The wire requires only that a `rev`
 * be a string ({@link parseBootManifest}), and a composer is free to mint its
 * own: the Host assigns initial rows an opaque per-boot nonce and content-hashes
 * only after an HMR rebuild, so two composers of the same graph legitimately
 * disagree on every rev. Hashing stays with the Host, which owns the served
 * artifacts it addresses.
 * @module @deepseek-ai/dsh-client-modules/graph
 */
...
export const PARSER_PRELOAD_IDS = [CLIENT_MODULES_ID] as const
