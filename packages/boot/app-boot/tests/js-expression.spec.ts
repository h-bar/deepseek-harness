...
describe('shipped `disabled` expressions under a browser scope', () => {
  ...
  // An expression reaching past `process.platform` is host-side, and fails
  // here rather than at that composer's boot.
  ...
  const browserScope = { process: { platform: 'browser' } }
